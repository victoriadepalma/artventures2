...
export const AgregarObra= ({show,setShow}) => {
  ...
  const agregar = async(e) => {
    e.preventDefault();
       if ( !values.tittle || !values.type || !values.year || !values.tecnica || !values.dimensiones || !values.description || !values.ID_ubicacion || !values.ID_tour || !values.ID_artista || !file) {
      setErrorMsg("Llene todos los campos");
      return;
    }
    setErrorMsg("");
dispatch(addObra({data:values,file:file}))
...
setFile("");setShow()}} />
      <div className={styles.modalContent}>
        ...
        <input placeholder="Título"  value={values.name} onChange={(e)=>{ setvalues((prev) => ({ ...prev, tittle: e.target.value }))}}/>
        </div>
        <div className={styles.input}>
